...
app.get('/darkthrone', (req, res) => {
  res.status(200).send({message : "hi"})
});

app.post('/:bandName', (req, res) => {
  const { bandName } = req.params;
  result = getRecommendations(bandName);
  console.log(result);
  res.status(200).send({result});
});

const getRecommendations = (bandName) => { //need to parse input
...
const getBandInfo = (bandLink) => {

};

const getSimilarBands = (similarBandsLink) => {
